...
const chats = new Datastore({
  filename: path.resolve(__dirname, 'database/chats.db'),
  autoload: true,
});

const profile = new Datastore({
  filename: path.resolve(__dirname, 'database/profile.db'),
  autoload: true,
});
...
io.on('connection', (socket) => {
  ...
  socket.on('chat message', (body) => {
    ...
    chats.update({ _id: message.chatId }, { $push: { messages: message } }, {}, () => {
